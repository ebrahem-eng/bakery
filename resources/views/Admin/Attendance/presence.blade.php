...
@if(!$activeWorkDay)
    <div class="glass-panel p-12 rounded-3xl border border-white/5 text-center">
        ...
        <a href="{{ route('admin.work_days.index') }}" class="inline-flex items-center px-6 py-3 bg-amber-500 hover:bg-amber-400 text-black font-bold rounded-xl transition-all shadow-xl">
             {{ __('Go to Work Days') }}
        </a>
    </div>
@else
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="open" x-transition 
                ...
                <form :action="`/admin/attendance/${attendanceId}/mark-departure`" method="POST" class="space-y-4">
                    ...
                    <div class="pt-4">
                        <button type="submit" class="w-full bg-amber-500 hover:bg-amber-400 text-[#0f1115] px-4 py-3 rounded-xl text-sm font-bold transition-colors shadow-lg">
                            {{ __('Save Departure') }}
                        </button>
                    </div>
                </form>
                ...
                <form action="{{ route('admin.attendance.bulk_mark_attendance') }}" method="POST" class="space-y-4">
                    ...
                    <div class="pt-4">
                        <button type="submit" 
                                ...
                                class="w-full px-4 py-3 rounded-xl text-sm font-bold transition-all shadow-lg flex items-center justify-center gap-2">
                            {{ __('Save Arrival') }}
...
<!-- Presence History Modal (Always available) -->
                        ...
                        @foreach($workers as $w)
                            @php
                                $presenceDates = \App\Models\WorkerAttendance::where('worker_id', $w->id)
                                    ->latest('arrival_time')
                                    ->get()
                                    ->groupBy(fn($a) => $a->arrival_time?->translatedFormat('Y-m-d') ?? 'N/A');
                            @endphp
                            <tr>
