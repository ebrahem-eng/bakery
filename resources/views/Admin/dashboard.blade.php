...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    <div class="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-white/5 relative overflow-hidden group">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mb-1">{{ __('Live Sales Volume') }}</p>
        <h3 class="text-3xl font-black text-slate-900 dark:text-white">{{ number_format($todaySales, 2) }} <span class="text-sm text-emerald-600 dark:text-emerald-400">{{ $defaultCurrency->code ?? 'USD' }} / {{ __('Gross') }}</span></h3>
    ...
    <div class="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-white/5 relative overflow-hidden group">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mb-1">{{ __('Live Cost / Expenses') }}</p>
        <h3 class="text-3xl font-black text-slate-900 dark:text-white">{{ number_format($todayExpenses, 2) }} <span class="text-sm text-red-600 dark:text-red-400">{{ $defaultCurrency->code ?? 'USD' }} / {{ __('Out') }}</span></h3>
    ...
    <div class="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-white/5 relative overflow-hidden group">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mb-1">{{ __('Registered Workers') }}</p>
        <h3 class="text-3xl font-black text-slate-900 dark:text-white">{{ number_format($totalWorkers) }}</h3>
    ...
    <div class="glass-panel p-6 rounded-2xl border border-slate-200 dark:border-white/5 relative overflow-hidden group">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mb-1">{{ __('Sales Channels') }}</p>
        <h3 class="text-3xl font-black text-slate-900 dark:text-white">{{ number_format($totalDistributors) }}</h3>
            ...
            <tbody class="divide-y divide-white/5">
                @forelse($lastDays as $day)
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                        ...
                        <td class="p-4">
                            <span class="text-xs text-slate-700 dark:text-slate-300 font-medium px-2.5 py-1 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded border">
