    ...
    <div class="lg:col-span-1">
        ...
        <div class="glass-panel p-6 rounded-3xl border border-white/5 sticky top-8"
             x-data="{ 
                ...
                workers: @js($workers),
                get selectedWorker() {
                    return this.workers.find(w => w.id == this.selectedWorkerId);
                }
             }">
            ...
            <form action="{{ route('admin.wages.store') }}" method="POST" class="space-y-4">
                ...
                <div x-show="selectedWorkerId" x-transition class="p-4 bg-white/5 border border-white/5 rounded-2xl flex justify-between items-center">
                    <div>
                        <p class="text-[10px] text-slate-500 uppercase font-bold tracking-widest">{{ __('Default Salary') }}</p>
                        <p class="text-white font-bold">
                            <span x-text="selectedWorker?.daily_wage"></span>
                            <span class="text-[10px] text-slate-400 font-medium ml-1" x-text="selectedWorker?.currency?.code"></span>
                        </p>
                    </div>
                    <div class="text-right">
                        <p class="text-[10px] text-slate-500 uppercase font-bold tracking-widest">{{ __('Shifts Today') }}</p>
                        <p class="text-emerald-500 font-black text-lg" x-text="selectedWorker?.shifts_count || 0"></p>
                    </div>
                </div>
