...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{{ __('Update Worker Profile') }}</h1>
    <p class="text-sm text-slate-500 mt-1">{{ __('Adjust credentials, mobile contacts, and fixed wages.') }}</p>
</div>

<form action="{{ route('admin.workers.update', $worker->id) }}" method="POST" class="glass-panel p-6 rounded-2xl max-w-4xl" 
    x-data="{ 
        mobiles: {{ $worker->mobiles->count() ? collect($worker->mobiles->pluck('mobile_number'))->toJson() : "['']" }},
        currencyId: '{{ old('currency_id', $worker->currency_id) }}',
        currencies: {{ $currencies->map(fn($c) => ['id' => $c->id, 'is_default' => $c->is_default, 'rate' => $c->exchange_rate, 'code' => $c->code])->toJson() }},
        showExchangeRate() {
            const c = this.currencies.find(i => i.id == this.currencyId);
            return c && !c.is_default;
        },
        getDefaultRate() {
            // If the selected currency is the same as the worker's original currency, use his saved rate as default
            if(this.currencyId == '{{ $worker->currency_id }}') return '{{ $worker->exchange_rate }}';
            const c = this.currencies.find(i => i.id == this.currencyId);
            return c ? c.rate : 1;
        }
    }">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div>
            ...
            <input type="text" name="first_name" value="{{ old('first_name', $worker->first_name) }}" required class="glass-input block w-full px-4 py-3 rounded-xl focus:outline-none focus:ring-1 focus:ring-[#0ea5e9]">
        ...
        <div>
            ...
            <input type="text" name="last_name" value="{{ old('last_name', $worker->last_name) }}" required class="glass-input block w-full px-4 py-3 rounded-xl focus:outline-none focus:ring-1 focus:ring-[#0ea5e9]">
        ...
        <div class="md:col-span-2">
            ...
            <input type="text" name="title" value="{{ old('title', $worker->title) }}" class="glass-input block w-full px-4 py-3 rounded-xl focus:outline-none focus:ring-1 focus:ring-[#0ea5e9]" placeholder="{{ __('e.g. Baker, Distributor, Guard') }}">
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/5 p-4 rounded-xl">
        <div>
            ...
            <input type="number" step="0.01" name="daily_wage" value="{{ old('daily_wage', $worker->daily_wage) }}" required class="glass-input block w-full px-4 py-3 rounded-xl focus:outline-none focus:ring-1 focus:ring-emerald-500 border border-emerald-200 dark:border-emerald-500/30">
        ...
        <div>
            ...
            <select name="currency_id" x-model="currencyId" required class="glass-input block w-full px-4 py-3 rounded-xl focus:outline-none focus:ring-1 focus:ring-emerald-500 border border-emerald-200 dark:border-emerald-500/30">
                ...
                @foreach($currencies as $currency)
                    <option value="{{ $currency->id }}" {{ old('currency_id', $worker->currency_id) == $currency->id ? 'selected' : '' }}>{{ $currency->name }} ({{ $currency->code }})</option>
                @endforeach
            </select>
        ...
        <div x-show="showExchangeRate()" x-transition class="md:col-span-2 p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl">
            <label class="block text-sm font-bold text-amber-600 dark:text-amber-400 mb-2">{{ __('Custom Exchange Rate') }} (1 <span x-text="currencies.find(c => c.id == currencyId)?.code || ''"></span> = ?? {{ __('SYP') }})</label>
            <div class="flex items-center gap-4">
                <input type="number" step="0.01" name="exchange_rate" :value="getDefaultRate()" class="glass-input flex-1 px-4 py-3 rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-500 border border-amber-500/30" placeholder="{{ __('Enter value in SYP') }}">
                <p class="text-xs text-slate-500 max-w-[200px] leading-relaxed">
                    {{ __('Please enter the converted value in SYP for this wage agreement.') }}
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="flex justify-end gap-4 border-t border-slate-200 dark:border-white/5 pt-6">
        ...
        <button type="submit" class="bg-gradient-to-r from-[#0ea5e9] to-[#3b82f6] hover:from-[#38bdf8] hover:to-[#60a5fa] text-white transition-all px-8 py-2.5 rounded-xl text-sm font-bold shadow-[0_0_20px_rgba(14,165,233,0.3)]">{{ __('Update Worker Profile') }}</button>
    </div>
</form>
@endsection
