...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-white tracking-tight">{{ __('Register Supplier') }}</h1>
    <p class="text-sm text-slate-400 mt-1">{{ __('Add a new vendor and assign their supplied materials.') }}</p>
</div>

<form action="{{ route('admin.suppliers.store') }}" method="POST" class="glass-panel p-6 rounded-2xl border border-white/5 max-w-4xl" x-data="{ mobiles: [''] }">
    @csrf
    ...
    <div class="mb-8">
        ...
        <div class="flex flex-wrap gap-4">
            ...
            <label class="flex items-center space-x-2 {{ app()->getLocale() == 'ar' ? 'space-x-reverse' : '' }} cursor-pointer">
                <input type="checkbox" name="categories[]" value="{{ $category->id }}" class="form-checkbox h-4 w-4 text-[#eab308] rounded border-white/10 bg-black/30 focus:ring-[#eab308] focus:ring-offset-slate-900 transition-colors">
    ...
    <div class="flex justify-end gap-4 border-t border-white/5 pt-6">
        ...
        <button type="submit" class="bg-[#eab308]/10 text-[#eab308] border border-[#eab308]/30 hover:bg-[#eab308] hover:text-[#451a03] transition-all px-6 py-2.5 rounded-xl text-sm font-bold shadow-[0_0_15px_rgba(234,179,8,0.15)]">{{ __('Register Supplier') }}</button>
    </div>
</form>
@endsection
