...
<div class="mb-6 glass-panel rounded-2xl border border-slate-200 dark:border-white/5 overflow-hidden">
    <div class="p-4 bg-slate-50 dark:bg-black/20 border-b border-slate-200 dark:border-white/5 flex justify-between items-center">
        ...
        <button @click="search = ''; filterCat = ''; date_from = ''; date_to = ''; currentPage = 1" class="text-[10px] uppercase tracking-widest font-bold text-slate-500 hover:text-amber-500 transition-colors">{{ __('Reset All') }}</button>
    </div>
    <div class="p-4 grid grid-cols-1 sm:grid-cols-4 gap-4">
        ...
        <div>
            ...
            <div class="flex items-center gap-2">
                <input type="date" x-model="date_from" @change="currentPage = 1" class="block w-full px-3 py-2 bg-white dark:bg-[#0f1115] border border-slate-200 dark:border-white/5 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-red-500/50 transition-all">
                <span class="text-slate-400 text-xs">-</span>
                <input type="date" x-model="date_to" @change="currentPage = 1" class="block w-full px-3 py-2 bg-white dark:bg-[#0f1115] border border-slate-200 dark:border-white/5 rounded-xl text-xs text-slate-900 dark:text-white focus:outline-none focus:border-red-500/50 transition-all">
            </div>
        </div>
        ...
        <table class="w-full text-start border-collapse">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-white/5">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors table-row-item"
                    ...
                    data-date="{{ $expense->created_at->format('Y-m-d') }}"
