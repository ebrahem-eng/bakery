...
class WorkerController extends Controller
{
    ...
    public function show(Worker $worker, Request $request)
    {
        ...
        // ── Contribution Stats (Revenue generated by worker) ──────────
        $totalRevenueSYP = $shifts->sum(fn($s) => $s->cash_collected_base);
        $totalBundlesSold = $shifts->sum(fn($s) => $s->bundles_received - $s->bundles_returned);
        $shiftCount = $shifts->count();
        $avgRevenueSYP = $shiftCount > 0 ? $totalRevenueSYP / $shiftCount : 0;
        ...
        return view('Admin.Workers.show', [
            ...
            'totalRevenueSYP' => $totalRevenueSYP,
            'totalBundlesSold' => $totalBundlesSold,
            'avgRevenueSYP' => $avgRevenueSYP,
            'shiftCount' => $shiftCount,
            'startDate' => $startDate,
            'endDate' => $endDate,
            'type' => $type,
            'shifts' => $shifts, // For the detailed performance table
        ]);
    }
